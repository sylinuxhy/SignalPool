...
class Rbreakweek:
    ...
    def __init__(self, sid, pid, df, uid=None, obs=0.35, rev=0.07, bre=0.25):
        ...
        self.opentime = 0
        data = getKline(self.df, interval='week')
    ...
    def algorithm(self, ticker):
        ...
        if self.position == 0:
            ...
            if (self.position == 1) and (self.rBreak != 1):
            ...
            elif (date >= datetime(year=self.opentime.year, month=self.opentime.month, day=self.opentime.day + 7,
                                   hour=23,
                ...
                if self.position == 1:
                    ...
                    if self.rBreak == 1:
                        self.rBreak = 0
                        content = "持有的{}价位的多单，以收盘前5分钟时间{} ({})的价格{}平仓".format(str_tome, self.breakBuyprice, str_time,
                                                                              price)
                    ...
                    self.position = 0
                    self.opentime = 0
                    Userchoose().choose(self.uid, self.pid)
                    if self.rBreak == -1:
                        self.rBreak = 0
